...
public class PopupPhoto extends AppCompatActivity {


    private static final String LOG = PopupPhoto.class.getName();

    private Button btnActivatePhoto;
    int id;
    public DatabaseHelper db,dbhelper;
    private TextView tvPopupTitle;
    private Toolbar toolbar;
    //Photo_model pm;
    Photo_model pmPP = new Photo_model();
...
//                    "\n Process Time: " + pmPP.getProcess_time();
            ...
            tvPopupTitle.setText(pmPP.getSummaryString() + "\nTableName: " + db.getArray(pmPP.getTimes_id(),UserID).getName() + "\nTimesArray: " + db.getArray(pmPP.getTimes_id(),UserID).getTimesString());
            ...
            public boolean onMenuItemClick(MenuItem item) {
                switch(item.getItemId()){
                    case R.id.item_activate:
                        ...
                        ((MyApplication) getApplication()).setJSONUrl(pmPP);
                            ((MyApplication) getApplication()).setActivatedType("photo");
//                        Toast.makeText(getApplicationContext(),"Activated profile: " + pmPP.getName() + "\nDirection: " + pmPP.getDirectionAsString() + "\n Speed: " + pmPP.getSpeed() +
//                                "\n Process Time: " + pmPP.getProcess_time() + "\nTimesID: " + pmPP.getTimes_id(),Toast.LENGTH_SHORT).show();

                        Intent Main = new Intent(PopupPhoto.this,MainActivity.class);
//                        Main.putExtra("activate_now",true);
                        startActivity(Main);
                            PopupPhoto.this.finish();
                        }
                        catch(Exception e)
                        {
                            e.printStackTrace();
                        ...
                        break;
                    case R.id.item_edit:
                        Intent EditIntent = new Intent(PopupPhoto.this,AddPhotoProfile.class);
                    ...
                    case R.id.item_delete:
                        try {
                            AlertDialog.Builder altBx = new AlertDialog.Builder(PopupPhoto.this);
                            altBx.setTitle("Deleting..");
                            altBx.setMessage(
                                    "Profile \"" + pmPP.getName() + "\" will be removed.\n" +
                                    "Are you sure?");
                            ...
                            altBx.setPositiveButton("Yes", new DialogInterface.OnClickListener() {
                                public void onClick(DialogInterface dialog, int which) {
                                    ...
                                    dbhelper.deletePhotoByID(id,(int)UserID);
                                    // Second - from the list ( that's why is that list is static )
                                    PhotoActivity.adapterPhoto.remove(pmPP.getName());
                                    PhotoActivity.adapterPhoto.notifyDataSetChanged();
                                    Toast.makeText(getApplicationContext(), pmPP.getName() + " successfully removed.", Toast.LENGTH_SHORT).show();
                                    PopupPhoto.this.finish();
                                }
                            });
